...
export class TrackFilesComponent {
  ...
  handleEdit() {
    ...
    this.editedContent =
      this.trackFilesService.selectedTrackFile?.comment || this.lorem;
  ...
  areAllFilteredSelected() {
    return this.trackFilesService.tableService.data.every(
      (row) => row.selected || (row.filtered && !row.selected)
    );
  }
}
